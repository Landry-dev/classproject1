    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <form action="" method="post">
                ...
                <input type="submit" name="add" value="Add Stock Out" class="btn btn-success">
            </form>
        </div>
    </div>
</div>
...
if (isset($_POST['add'])) {
    ...
    $sql = "INSERT INTO productout (ProductCode, Date, Quantity, UniquePrice, TotalPrice) 
...
if (isset($_POST['add'])) {
    ...
    // $available_stock = $check_row['available'];
    // if ($available_stock < $quantity) {
    //     echo "Error: Only $available_stock items available (tried to remove $quantity)";
    //     exit;
    // }

    if ($check_row['available'] < $quantity) {
        $_SESSION['error'] = "Not enough stock available! Only {$check_row['available']} units in stock.";
        header("Location: add_stock_out.php");
        exit();
    }
    ...
    if ($result) {
        ...
        echo "Failed to add stock out: " . mysqli_error($conn);
    }
}
?>